...
import { headersData_xm_cprog } from "../headers/headersTables.js";
...
const Data_xm_cprog = () => {
  ...
  const [eliminarData_xm_cprog] = useMutation(ELIMINAR_DATA_XM_CPROG, {
    update(cache) {
      ...
      cache.writeQuery({
        query: OBTENER_DATA_XM_CPROG,
        data: {
          obtenerData_xm_cprog: obtenerData_xm_cprog.filter(
            (obtenerData_xm_cprog) => obtenerData_xm_cprog.id !== id1.toString()
          ),
        },
      });
    },
  ...
  const headers = [
    ...
    { name: "Agente", field: "agente", sortable: true },
    {
      name: "Cargo_Cprog_Cop_Kwh",
      field: "cargo_cprog_cop_kwh",
      sortable: true,
    },
    { name: "Createdat", field: "createdAt", sortable: true },
    { name: "Updatedat", field: "updatedAt", sortable: true },
  ];
  ...
  useEffect(() => {
    ...
    var data_xm_cprogm = data_xm_cprog.filter(
      (data_xm_cprog) =>
        data_xm_cprog.empresa_id === data1.obtenerUsuario.empresa
    );

    data_xm_cprogm = data_xm_cprogm.sort(function (a, b) {
      if (a.anho === b.anho) {
  ...
  const confirmarEliminarRegistro = (eliminarData_xm_cprogId) => {
    ...
    const data_xm_cprog = data.obtenerData_xm_cprog;
    var data_xm_cprog_eliminar = data_xm_cprog.filter(
      (data_xm_cprog) =>
        data_xm_cprog.empresa_id === data1.obtenerUsuario.empresa &&
        data_xm_cprog.id === eliminarData_xm_cprogId
    );
                ...
                <div className="col mb-3 col-12 text-center">
                  ...
                  <div className="col-md-6">
                    <div className="text-left mr-0 mb-3">
                      ...
                      >
                        <i className="fa fa-plus mr-2 white"></i>Agregar Datos
                      </button> */}
                      {/* <NuevoData_xm_cprog
                        show={showLogin}
                      ...
                      <AddData
                        ...
                        subMutation="nuevoData_xm_cprog"
                        inputFields={headersData_xm_cprog}
                        cacheField="obtenerData_xm_cprog"
                        tituloTabla="Agregar Datos CPROG"
                        linkEjemplo=""
                      />
                    </div>
                    <Paginacion
