// Importar los modelos necesarios (ajusta las rutas según tu estructura)
const Data_dane_ipp = require('../models/Data_dane_ipp');
const Data_dane_ipc = require('../models/Data_dane_ipc');
...
// Función helper para calcular la energía de contratos a partir de un array de objetos
const calcularEnergiaContratos = (data) => {
...
// Función helper para calcular el costo de contratos a partir de un array de objetos
const calcularCostoContratos = (data) => {
  return data.reduce((total, obj) => {
    for (let i = 1; i <= 24; i++) {
      total += parseFloat(obj[`desp_hora_${i}`]) * parseFloat(obj[`trf_hora_${i}`]);
    }
    return total;
  }, 0);
};
...
/**
 * Función que realiza los cálculos para componentes de tarifa.
 * Solo se reciben del input: anho, mes y mercado.
 * Se utiliza un valor fijo para empresa_id (por ejemplo, 1) ya que no viene en el input.
 *
 * @param {Object} param0 - Objeto con parámetros de entrada.
 * @param {number} param0.anho - Año actual.
 * @param {number} param0.mes - Mes actual.
 * @param {string|number} param0.mercado - Valor del mercado.
 * @returns {Object} - Objeto con los valores calculados.
 */
const calcularComponentesTarifa = async (input, ctx) => {
        ...
        for (let index = 0; index < input.length; index++) {
          try {
            ...
            try {
              ...
              console.log("Energia_contratos_sub", Energia_contratos_sub);
              qc_ = roundToTwo(
                Math.min(
                  1 - qagd,
