...
if (data[1].Definitivo==='Mes cargo:') {
    const fecha=(new Date(((Math.floor((data[1].__EMPTY - 25568)*86400*1000)))).toLocaleDateString("en-US"))
    setMes(parseFloat(fecha.substr(0,2)))
    setAnho(parseFloat(fecha.substr(5,4)))   
};
if (data[6].__EMPTY_2==='Cargo CPROG por concepto del plan ($/kWh)') {
    setCargo_Cprog_Cop_Kwh(data[7].__EMPTY_2)
 };
...
headers[j] = headers[j-1].replace('tasa', 'monto')
}
}
}
