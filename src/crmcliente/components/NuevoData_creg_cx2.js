...
if (j >1){
if (encabezados[j-2] === 'Fecha' || encabezados[j-2] === 'Empresa_Id' || encabezados[j-2] === 'Resolucion' )
...
return {fecha:(item["Fecha"]),creador:creador,empresa_id:empresa_id,resolucion:(item["Resolucion"]),mo:parseFloat(item["Mo"]),RCT:parseFloat(item["RCT"]),RCAE:parseFloat(item["RCAE"]),RCSNE:parseFloat(item["RCSNE"]),RCNU:parseFloat(item["RCNU"]),Cf:parseFloat(item["Cf"]),PUI:parseFloat(item["PUI"])}
});
console.log(arreglado)
const {results} = await Promise.all(arreglado.map(object => {
return nuevoData_creg_cx({ variables:{
input:
object
}
});
...
<Modal.Title>Cargue Masivo a tabla Data CREG CX</Modal.Title>
</Modal.Header>
<Modal.Body>
<div>
...
export default NuevoDatacreg_cx2
