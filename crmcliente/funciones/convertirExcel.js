function mesANumero(mes) {
    const mesSinEspacios = mes.toUpperCase().replace(/\s/g, "");
    const equivalencias = {
      ENERO: 1,
      FEBRERO: 2,
      MARZO: 3,
  ...
  export function convertirSTR(sheetName, excelArray, dataArray2) {
    ...
    switch (true) {
      ...
      case sheetName.includes("CargosEstimados Oct-2023"):
       console.log('first')
        excelArray.forEach((row) => {
          let lastValueIndex = row.lastIndexOf("") - 2; // Encuentra el último valor no vacío en la fila
          if (lastValueIndex < 0) {
          ...
          switch (row[0]) {
            case "TOTAL INGRESO MENSUAL BRUTO STR -  (COP)":
              resultObject.total_ingreso_mensual_bruto_str_cop_norte = parseFloat(
                row[1].replace(/,/g, "")
              );
            ...
            case "ENERGÍA DEL STR (kWh)":
              ...
              break;
            case "VALOR DEL CARGO ANTES DE COMPENSACIÓN ($/kWh )":
              ...
              resultObject.cargo_nt_antes_de_compensacion_cd4_cop_kwh_sur =
                parseFloat(row[4].replace(/,/g, ""));
              break;
  
            case "VALOR DEL CARGO DESPUÉS DE COMPENSACIÓN ($/kWh )":
              ...
              resultObject.cargo_nt_despues_de_compensacion_cd4_cop_kwh_sur =
                parseFloat(row[4].replace(/,/g, ""));
              break;
            case "Cargo por uso del nivel de tensión 4  ($/kWh )":
