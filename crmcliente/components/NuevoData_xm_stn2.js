...
const NuevoDatadataxmstn2 = (props) => {
  ...
  const [t_cop_kwh, setT_Cop_Kwh] = useState("");
  const [t_prima_cop_kwh, setT_Prima_Cop_Kwh] = useState("");
  const [Energia_sin_kwh, setEnergia_Sin_Kwh] = useState("");
  const [Ing_Reg_Bruto_T_cop, setIng_Reg_Bruto_T_Cop] = useState("");
  const [Ing_Compensar_T_cop, setIng_Compensar_T_Cop] = useState("");
  const [Ing_Reg_Neto_T_cop, setIng_Reg_Neto_T_Cop] = useState("");
  const [delta_t_cop_kwh, setDelta_T_Cop_Kwh] = useState("");
  const [Agente, setAgente] = useState("");
  const [Demanda_kWh, setDemanda_Kwh] = useState("");
  const [CRS_Variable_Guatape_Cop, setCrs_Variable_Guatape_Cop] = useState("");
  ...
  const onDrop = useCallback((acceptedFiles) => {
    ...
    reader.onload = () => {
      ...
      const excelRows = XLSX.utils.sheet_to_row_object_array(
        workbook.Sheets["Cargos Monomios"]
      );
      const data = excelRows;

      if (
        data[0].__EMPTY_2 ===
        "Ingreso Regulado Bruto que pagan los comercializadores ($) ="
      ) {
      ...
      if (
        data[7].__EMPTY_3 === "TOTAL" &&
        data[7].__EMPTY_2 === "DEMANDA\nMÍNIMA"
      ) {
      ...
      const excelRows2 = XLSX.utils.sheet_to_row_object_array(
        workbook.Sheets["Deltas"]
      );
      ...
      const excelRows3 = XLSX.utils.sheet_to_row_object_array(
        workbook.Sheets["CRS_Variante Guatape"]
      );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { data } = await nuevoDataxmstn({
        variables: {
          input: {
            ...
            anho: parseInt(fileNames[0].substr(11, 4)),
            mes: parseInt(fileNames[0].substr(16, 2)),
            t_cop_kwh: t_cop_kwh,
            t_prima_cop_kwh: t_prima_cop_kwh,
            Energia_sin_kwh: Energia_sin_kwh,
            Ing_Reg_Bruto_T_cop: Ing_Reg_Bruto_T_cop.toString(),
            Ing_Compensar_T_cop: Ing_Compensar_T_cop.toString(),
            Ing_Reg_Neto_T_cop: Ing_Reg_Neto_T_cop.toString(),
            delta_t_cop_kwh: delta_t_cop_kwh,
          },
        },
      });
      var arreglado = datacsv.map((item) => {
        return {
          creador: creador,
          agente: item["Participante del Mercado"],
          demanda_kwh: item["DEMANDA REAL TOTAL (kWh)"],
          crs_variable_guatape_cop: item["CRS_VARIANTE GUATAPE (COP)"],
          empresa_id: empresa_id,
          anho: parseInt(fileNames[0].substr(11, 4)),
          mes: parseInt(fileNames[0].substr(16, 2)),
        };
      ...
      const { results } = await Promise.all(
        arreglado.map((object) => {
          return nuevoData_xm_guatape({
            variables: {
              input: object,
            },
          });
