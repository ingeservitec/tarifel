...
const NUEVO_DATA_XM_TRSM= gql`
mutation nuevoDataxm_trsm($input:Dataxm_trsmInput ){
nuevoDataxm_trsm(input:$input){
  anho
  mes
  creador
  empresa_id
...
const NuevoDataxm_trsm2= (props) => {
const { data:data1, error:error1, loading:loading1} = useQuery(OBTENER_USUARIO);
const [datacsv, setDatacsv] = useState("");
const [fileNames, setFileNames] = useState([]);
const [creador, setCreador] = useState();
const [empresa_id, setEmpresa_id]= useState("")
const [nuevoDataxm_trsm]=useMutation(NUEVO_DATA_XM_TRSM, {
update(cache, { data: { nuevoDataxm_trsm} } ) {
...
data: {
obtenerData_xm_trsm: [...obtenerData_xm_trsm, nuevoDataxm_trsm]
}
})
}
})
...
// Parse CSV file
   ...
   var delimiter = ";"; // user-supplied delimeter or default comma
   ...
   while (matches = pattern.exec( reader.result )) {
...
console.log("reader.result CSV data: ", rows);

console.log("rACA");
console.log("reader.result CSV data: ", reader.result);
setDatacsv(rows)
...
if (j >1){
if (obj[headers[j-2]] ='FECHA')
obj[headers[j-2]] = (currentline[j-2]);
if (obj[headers[j-2]] ='CODIGO')
obj[headers[j-2]] = (currentline[j-2]);
if (obj[headers[j-2]] ='DESCRIPCION')
obj[headers[j-2]] = (currentline[j-2]);
else{
...
return {anho:parseFloat(((item["FECHA"]).substr(0,4))),mes:parseFloat(((item["FECHA"]).substr(5,2))),creador:creador,empresa_id:empresa_id,fecha:(item["FECHA"]),codigo:(item["CODIGO"]),descripcion:(item["DESCRIPCION"]),valor:parseFloat(item["VALOR"])}
});
console.log(arreglado)
const {results} = await Promise.all(arreglado.map(object => {
return nuevoDataxm_trsm({ variables:{
input:
object
}
});
...
<Modal.Title>Cargue Masivo a tabla Data TRSM</Modal.Title>
</Modal.Header>
<Modal.Body>
<div>
...
export default NuevoDataxm_trsm2