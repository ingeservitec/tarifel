...
const NuevoDataxm_cprog2 = (props) => {
  ...
  const [cargo_cprog_cop_kwh, setCargo_Cprog_Cop_Kwh] = useState("");
    ...
    acceptedFiles.forEach((file) => {
      ...
      reader.onload = () => {
        try {
          ...
          if (workbook.Sheets["CargoCPROG"]) {
            const excelRows = XLSX.utils.sheet_to_row_object_array(
              workbook.Sheets["CargoCPROG"]
            );
            const data = excelRows;
            console.log(data);
            // Tu lógica de procesamiento aquí
          } else {
            console.log("La hoja CargoCPROG no existe en el archivo.");
          }
        } catch (error) {
      ...
      const { data } = await nuevoData_xm_cprog({
        variables: {
          input: {
            ...
            mes: mes,
            agente: agente,
            cargo_cprog_cop_kwh: cargo_cprog_cop_kwh,
          },
        },
      });
