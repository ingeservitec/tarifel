...
const NuevoData_mme_giro = (props) => {
  ...
  const [link_resolucion, setLink_Resolucion] = useState("");
  const [giro_cop, setGiro_Cop] = useState("");
  const [nuevoData_mme_giro] = useMutation(NUEVO_DATA_MME_GIRO, {
    update(cache, { data: { nuevoData_mme_giro } }) {
      // Obtener el objeto de cache que deseamos actualizar
      const { obtenerData_mme_giro } = cache.readQuery({
  ...
  function csvJSON(csv2) {
    ...
    for (var i = 1; i < lines.length; i++) {
      ...
      for (var j = 0; j < headers.length + 2; j++) {
        ...
        if (j > 1) {
          if (
            encabezados[j - 2] === "empresa_id" ||
            encabezados[j - 2] === "Fecha" ||
            encabezados[j - 2] === "Fondo" ||
            encabezados[j - 2] === "Resolucion" ||
            encabezados[j - 2] === "Link_Resolucion"
          )
  ...
  useEffect(() => {
    if (datacsv) {
      ...
      setFecha(datacsv[1][Position]);
      var Position = datacsv[0].indexOf("Fondo".toString());
      setFondo(datacsv[1][Position]);
      var Position = datacsv[0].indexOf("Resolucion".toString());
      setResolucion(datacsv[1][Position]);
      var Position = datacsv[0].indexOf("link_resolucion".toString());
      setLink_Resolucion(datacsv[1][Position]);
      var Position = datacsv[0].indexOf("giro_cop".toString());
      setGiro_Cop((datacsv[1][Position]).toString());
    } else {
    }
  }, [datacsv]);
      ...
      >
        ...
        <Modal.Body>
          ...
          <form onSubmit={formik.handleSubmit}>
            ...
            <div className="form-group row">
              <label htmlFor="fecha" className="col-sm-7 col-form-label">
                Fecha
              </label>
              <div className="col-sm-5">
                <input
            ...
            <div className="form-group row">
              <label htmlFor="fondo" className="col-sm-7 col-form-label">
                Fondo
              </label>
              <div className="col-sm-5">
                <select
                ...
                >
                  ...
                  <option value="FSSRI">FSSRI</option>
            ...
            <div className="form-group row">
              <label htmlFor="resolucion" className="col-sm-7 col-form-label">
                Resolucion
              </label>
              <div className="col-sm-5">
                <input
                  ...
                  onChange={formik.handleChange}
                  onBlur={formik.handleBlur}
                  value={formik.values.resolucion}
                ></input>
              </div>
            </div>
            ...
            <div className="form-group row">
              ...
              <div className="col-sm-5">
                <input
                  ...
                  className="form-control"
                  id="link_resolucion"
                  placeholder="Link_Resolucion"
                  onChange={formik.handleChange}
                  onBlur={formik.handleBlur}
                  value={formik.values.link_resolucion}
                ></input>
              </div>
            </div>
            ...
            <div className="form-group row">
              <label htmlFor="giro_cop" className="col-sm-7 col-form-label">
                Giro_Cop
              </label>
              <div className="col-sm-5">
                <input
                  ...
                  className="form-control"
                  id="giro_cop"
                  placeholder="Giro_Cop"
                  onChange={formik.handleChange}
                  onBlur={formik.handleBlur}
                  value={formik.values.giro_cop}
                ></input>
              </div>
            </div>
