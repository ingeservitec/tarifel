...
import NuevoData_xm_ipr from './NuevoData_xm_ipr';
import NuevoData_xm_ipr2 from './NuevoData_xm_ipr2';
...
<NuevoData_xm_ipr show={showLogin} close={() => setShowLogin(false)} />
<button variant="primary" onClick={() => setShowLogin2(true)}><i className="fa fa-plus mr-2 white"></i>Agregar Datos Masivos</button>
<NuevoData_xm_ipr2 show2={showLogin2} close2={() => setShowLogin2(false)} />
</div>
<Paginacion
total={totalItems}
...
<td>{comment.mes}</td>
<td>{comment.strID}</td>
<td>{comment.agrupaORMercado}</td>
<td>{comment.fechavigencia}</td>
<td>{comment.conceptoID}</td>
<td>{comment.nivelEntrada}</td>
<td>{comment.nivelSalida}</td>
<td>{comment.valor}</td>
